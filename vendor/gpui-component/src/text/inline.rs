...
impl Inline {
    ...
    fn layout_selections(
        ...
        // Once the mouse button is released, retain the byte ranges selected
        // in each AST node. Recomputing them from the original viewport-local
        // rectangle would pin the highlight to the viewport while a
        // virtualized TextView scrolls.
        if !text_view_state.is_selecting() && !text_view_state.selection_needs_layout() {
