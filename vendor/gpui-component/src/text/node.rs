...
impl Paragraph {
    ...
    pub(super) fn clear_selection(&self) {
        for child in &self.children {
            child.state.lock().unwrap().selection = None;
        }
...
impl Node {
    ...
    pub(super) fn clear_selection(&self) {
        match self {
            ...
            Node::CodeBlock(code_block) => {
                code_block.state.lock().unwrap().selection = None;
            }
            Node::Definition { .. } | Node::Break { .. } | Node::Divider | Node::Unknown => {}
        }
    }
...
impl Node {
    pub(super) fn root_scroll_weights(&self) -> Vec<usize> {
        ...
        children
            ...
            .map(|child| match child {
                Node::CodeBlock(code_block) => code_block.code().lines().count().max(1),
                Node::Table(table) => table.children.len().max(1),
                Node::Blockquote { children }
                | Node::List { children, .. }
                | Node::ListItem { children, .. } => children.len().max(1),
                _ => 1,
            })
            .collect()
    }
